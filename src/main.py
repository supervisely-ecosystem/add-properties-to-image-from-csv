...
def read_csv_and_create_index():
    ...
    with open(csv_path_local, "r") as f:
        ...
        for row in reader:
            CSV_INDEX[row[CSV_COLUMN_NAME].strip()] = row
    ...
    for column_name in CSV_COLUMNS:
        ...
        if tag_meta is None:
            ...
            if RESOLVE == "skip":
                ...
                raise RuntimeError("Tag {!r} not found in project {!r}".format(tag_meta.name, PROJECT.name))
    api.project.update_meta(RES_PROJECT.id, RES_META.to_json())


def main():
    global PROJECT, RES_PROJECT
    PROJECT = api.project.get_info_by_id(PROJECT_ID)

    read_and_validate_project_meta()
    read_csv_and_create_index()

    RES_PROJECT = api.project.create(WORKSPACE_ID, project.name, change_name_if_conflict=True)
    my_app.logger.info("Result Project is created (name={!r}; id={!r})".format(RES_PROJECT.name, RES_PROJECT.id))
